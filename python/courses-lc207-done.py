...
def canComplete(num, courses):
    ...
    for i in range(num):
        # if not visited[i]:
        if isCycle(i, visited, deplist, cycled):
            print(str(i) + ", " + str(visited))
            return False
    return True


def isCycle(i, visited, deplist, cycled):
    if cycled[i] is False:
        return False
    if len(deplist[i]) == 0:
        return False
    visited[i] = True
    for j in deplist[i]:
        if visited[j] is True or isCycle(j, visited, deplist, cycled) is True:
            return True
    visited[i] = False
    cycled[i] = False
    return False
...
# test = canComplete(20, [[0,10],[3,18],[5,5],[6,11],[11,14],[13,1],[15,1],[17,4]])

test = canComplete(100,
